...
class LlmEmotionAnalyzer(EmotionAnalyzer):
    ...
    def _is_openai_reasoning_model(self) -> bool:
        ...
        DeepSeek には適用しない（OpenAI 互換だが thinking は extra_body で別管理）。
        """
    ...
    # ------------------------------------------------------------------
    # API calls（各プロバイダーの例外は原因の分かる日本語に変換して送出）
    # ------------------------------------------------------------------
    ...
    def _call_openai(
        ...
        if self._is_openai_reasoning_model():
            # 推論モデル(GPT-5 / o 系): 推論トークンも消費するため出力上限は大きめ、temperature 非対応。
            params["max_completion_tokens"] = max(4096, max_tokens) + 4096
            if self.reasoning_effort:
                params["reasoning_effort"] = self.reasoning_effort
        else:
            # 従来モデル(gpt-4o 系): max_tokens + temperature。
